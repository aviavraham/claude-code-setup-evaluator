...
**Required:**
- **acli (Atlassian CLI)**: https://developer.atlassian.com/cli
  ```bash
  # Authenticate
  acli auth login
  ```
...
```bash
acli jira workitem search --jql "reporter = currentUser()" --limit 1  # Should return a Jira issue
...
Use `acli` to fetch issues created in the quarter:

```bash
# Fetch issues created in Q1 2026 (search returns key, summary, type, status, priority)
acli jira workitem search \
  --jql "reporter = currentUser() AND created >= '2026-01-01' AND created <= '2026-03-31' AND project = {PROJECT}" \
  --paginate --json
```

**Note:** The `search` command does not return `created` or `resolutiondate` fields.
For each issue returned, fetch dates individually:

```bash
# Get created and resolved dates for cycle time calculation
...
- Created and resolved dates (from `view` output)
...
- Linked PRs (GitHub links from issuelinks or description)
- Sub-tasks (indicates large effort)
- Time to resolve (created → resolutiondate)
...
- Jira: Run `acli jira workitem search --jql "reporter = currentUser()" --limit 1 --json` and check the assignee email
...
- Jira: Run `acli auth login` to re-authenticate
- GitHub: Run `gh auth refresh`
- GitLab: Run `glab auth login`

**No data returned:**
- Verify CLI tools are authenticated (run `acli auth status`, `gh api user`)
...
**When to delegate:**
- Detailed Jira ticket investigation → Use acli or Jira skills
