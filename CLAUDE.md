...
5. **Write minimal, correct code** (Ponytail ladder) - Before writing any code, stop at the first rung that holds:
   ...
   **The ladder runs after understanding the problem**, not instead of it: read the task and code it touches, trace the real flow end-to-end, then climb. Mark deliberate simplifications with a `ponytail:` comment naming the ceiling and upgrade path. Never skip validation, error handling, security, or accessibility — these are never on the chopping block.
